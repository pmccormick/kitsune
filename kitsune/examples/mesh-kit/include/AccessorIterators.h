...
/**
 * @brief Iterator extensions for MeshAccessor
 * 
 * These extensions provide iterators for the MeshAccessor class
 * to enable range-based for loops and other iterator-based patterns.
 */
...
/**
 * @brief Iterator extensions for RegionAccessor
 * 
 * These extensions provide iterators for the RegionAccessor class.
 */
template <typename MeshType, typename CellType>
...
public:
	...
	bool isCellInRegion(const CellType* cell, RegionMask mask) const {
          // Get the region mask for this cell directly from the mesh
          RegionMask cellMask = m_mesh.getCellRegionMask(cell->i(), cell->j());
          // A cell is in the region if there's any overlap between the cell's mask and the region mask
          return (cellMask & mask) != 0;
        }
...
/**
 * @brief Iterator extensions for FieldAccessor
 * 
 * These extensions provide iterators for the FieldAccessor class.
 */
...
/**
 * @brief Iterator extensions for CompoundAccessor
 * 
 * These extensions provide iterators for the CompoundAccessor class.
 */
template <typename MeshType, typename CellType>
class CompoundAccessorIterators {
public:
    /**
     * @brief Iterator for cells in a region with a specific field
     * 
     * @tparam T Field data type
     * @tparam LocationTag Field location tag
     */
    ...
    /**
     * @brief Range class for cell-field pairs
     * 
     * @tparam T Field data type
     * @tparam LocationTag Field location tag
     */
