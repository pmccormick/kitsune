...
concept CellPredicate = requires(P pred, const C* cell) {
...
concept CellConcept = requires(C cell) {
    ...
    { cell.mesh() } -> std::convertible_to<MeshBase*>;
    
    // Ensure it inherits from CellBase
    requires std::is_base_of_v<CellBase, C>;
};
...
concept MeshConcept = requires(M mesh, int i, int j) {
    ...
    { mesh.getCell(i, j) } -> std::convertible_to<CellBase*>;
...
concept RegionOperation = requires(Op op, const Region& r1, const Region& r2) {
...
concept RegionTraversalStrategy = requires(T strategy, const Region& region, std::vector<C*>& cells) {
...
concept RegionIterator = requires(I it, I other) {
