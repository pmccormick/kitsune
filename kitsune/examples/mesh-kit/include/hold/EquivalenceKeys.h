...
inline bool doubleEquals(double a, double b, double tolerance) {
    ...
    if (std::isinf(a) && std::isinf(b)) {
        return (a > 0 && b > 0) || (a < 0 && b < 0); // Same-signed infinities are equal
    }
 ...
 * @details
 * 
 * The StandardEquivalenceKey class provides a configurable implementation of the
 * IMaterialEquivalenceKey interface, determining when two Material objects should be
 * considered functionally equivalent within a specified tolerance.
 * 
 * Key features:
 * 1. Configurable tolerance: Controls the precision of floating-point comparisons
 * 2. Selective property comparison: Allows enabling/disabling comparison of specific properties
 * 3. Property weighting: Assigns importance weights to different properties
 * 4. Customizable comparison aspects: Controls whether to compare type, name, etc.
 */
...
public:
    ...
    bool areEquivalent(const std::shared_ptr<Material>& a, 
        ...
        if (m_compareRefTemperature && 
            !doubleEquals(a->getReferenceTemperature(), b->getReferenceTemperature(), m_tolerance)) {
            return false;
        }
        ...
        for (size_t i = 0; i < static_cast<size_t>(Material::MaterialProperty::COUNT); ++i) {
            if (m_propertyFlags[i]) {
                ...
                if (!doubleEquals(a->getProperty(prop), b->getProperty(prop), m_tolerance)) {
                    return false;
                }
            }
        }
        ...
        if (m_compareMixtureComponents) {
            ...
            if (a->isMixture()) {
                ...
                // Compare components (order matters in the current Material implementation)
                for (size_t i = 0; i < componentsA.size(); ++i) {
                    if (!doubleEquals(componentsA[i].second, componentsB[i].second, m_tolerance)) {
                        return false;
                    }
    ...
    size_t hash(const std::shared_ptr<Material>& material) const override {
        ...
        for (size_t i = 0; i < static_cast<size_t>(Material::MaterialProperty::COUNT); ++i) {
            if (m_propertyFlags[i]) {
                auto prop = static_cast<Material::MaterialProperty>(i);
                h ^= hashDouble(material->getProperty(prop), m_tolerance) + i; // Add i to distinguish properties
            }
        }
    ...
    void setPropertyComparison(Material::MaterialProperty property, bool include) override {
...
protected:
    double m_tolerance;
    std::array<bool, static_cast<size_t>(Material::MaterialProperty::COUNT)> m_propertyFlags;
    std::array<double, static_cast<size_t>(Material::MaterialProperty::COUNT)> m_propertyWeights;
    bool m_compareType;
    bool m_compareName;
 ...
 * self-tuning implementation that automatically adjusts tolerance and comparison
 * settings based on observed material variations during simulation.
 * 
 * Key features:
 * 1. Automatic tolerance adjustment: Adapts to the observed distribution of property values
 * 2. Property-specific tolerances: Uses different tolerances for different properties
 * 3. Statistical analysis: Identifies meaningful variations vs. numerical noise
 * 4. Adaptive property selection: Automatically enables/disables properties based on significance
 */
...
public:
     ...
     */
    AdaptiveEquivalenceKey(double baseTolerance = 1e-6, size_t adaptationInterval = 1000)
    ...
    bool areEquivalent(const std::shared_ptr<Material>& a, 
        ...
        for (size_t i = 0; i < static_cast<size_t>(Material::MaterialProperty::COUNT); ++i) {
            if (m_propertyFlags[i]) {
                auto prop = static_cast<Material::MaterialProperty>(i);
                if (!doubleEquals(a->getProperty(prop), b->getProperty(prop), m_propertyTolerances[i])) {
                    return false;
                }
            }
        }
    ...
    size_t hash(const std::shared_ptr<Material>& material) const override {
        ...
        for (size_t i = 0; i < static_cast<size_t>(Material::MaterialProperty::COUNT); ++i) {
            if (m_propertyFlags[i]) {
                auto prop = static_cast<Material::MaterialProperty>(i);
                h ^= hashDouble(material->getProperty(prop), m_propertyTolerances[i]) + i;
            }
        }
    ...
    std::unordered_map<std::string, double> getStatistics() const override {
        ...
        return {
            {"Base Tolerance", m_baseTolerance},
            ...
            {"Hit Rate", m_comparisonCount > 0 ? (static_cast<double>(m_hitCount) / m_comparisonCount) * 100.0 : 0.0},
            {"Properties Compared", countPropertiesCompared()}
        };
    }
...
private:
    ...
    std::array<double, static_cast<size_t>(Material::MaterialProperty::COUNT)> m_propertyTolerances;
    ...
    mutable std::array<PropertyVariation, static_cast<size_t>(Material::MaterialProperty::COUNT)> m_propertyVariations;
    ...
    void adaptTolerances() const {
        ...
        for (size_t i = 0; i < static_cast<size_t>(Material::MaterialProperty::COUNT); ++i) {
            ...
            if (variation.count < 10) continue; // Need enough samples
 ...
 * @details
 * 
 * The DomainSpecificEquivalenceKey class extends StandardEquivalenceKey to provide
 * pre-configured settings optimized for different types of CFD simulations. It allows
 * users to quickly select appropriate comparison settings for their specific domain
 * without manual configuration.
 * 
 * Key features:
 * 1. Domain presets: Pre-configured settings for common CFD simulation types
 * 2. Physics-based weighting: Property importance weights based on physical significance
 * 3. Domain-appropriate tolerances: Tolerance settings matched to domain requirements
...
public:
     ...
     */
    DomainSpecificEquivalenceKey(DomainType domainType = DomainType::GENERAL, double tolerance = 1e-6)
...
private:
    ...
    void configureDomain(DomainType domainType) {
        ...
        switch (domainType) {
            case DomainType::THERMAL:
                ...
                m_propertyWeights[static_cast<size_t>(Material::MaterialProperty::THERMAL_CONDUCTIVITY)] = 2.0;
                m_propertyWeights[static_cast<size_t>(Material::MaterialProperty::SPECIFIC_HEAT)] = 2.0;
                m_propertyWeights[static_cast<size_t>(Material::MaterialProperty::THERMAL_EXPANSION)] = 2.0;
                break;
                
            case DomainType::FLUID_FLOW:
                // Emphasize flow properties
                m_propertyWeights[static_cast<size_t>(Material::MaterialProperty::DENSITY)] = 2.0;
                m_propertyWeights[static_cast<size_t>(Material::MaterialProperty::DYNAMIC_VISCOSITY)] = 2.0;
                
                // De-emphasize thermal properties
                m_propertyWeights[static_cast<size_t>(Material::MaterialProperty::THERMAL_CONDUCTIVITY)] = 0.5;
                break;
            ...
            case DomainType::MULTIPHASE:
                ...
                m_propertyWeights[static_cast<size_t>(Material::MaterialProperty::SURFACE_TENSION)] = 2.0;
                break;
                
            case DomainType::THERMAL_STRESS:
                // Emphasize mechanical and thermal properties
                m_propertyWeights[static_cast<size_t>(Material::MaterialProperty::THERMAL_EXPANSION)] = 2.0;
                m_propertyWeights[static_cast<size_t>(Material::MaterialProperty::THERMAL_CONDUCTIVITY)] = 1.5;
            ...
            case DomainType::HIGH_PERFORMANCE:
                ...
                m_propertyFlags[static_cast<size_t>(Material::MaterialProperty::ELECTRICAL_CONDUCTIVITY)] = false;
                m_propertyFlags[static_cast<size_t>(Material::MaterialProperty::SURFACE_TENSION)] = false;
    ...
    std::string getDomainName(DomainType type) const {
        switch (type) {
            case DomainType::THERMAL: return "Thermal";
            case DomainType::FLUID_FLOW: return "Fluid Flow";
            case DomainType::MULTIPHASE: return "Multiphase";
            case DomainType::THERMAL_STRESS: return "Thermal Stress";
            case DomainType::HIGH_PERFORMANCE: return "High Performance";
            case DomainType::GENERAL: return "General";
