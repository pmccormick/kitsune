 ...
 * @details
 * 
 * The AdaptiveEvictionPolicy class implements a sophisticated cache eviction strategy
 * that automatically selects between multiple underlying policies based on observed
 * access patterns. This approach combines the strengths of different eviction strategies
 * to optimize cache performance for changing workloads.
 * 
 * Key features:
 * 1. Pattern detection: Analyzes access history to identify sequential scans, loops, etc.
 * 2. Automatic policy selection: Switches between LRU, LFU, and time-sensitive policies
 * 3. Multiple policy tracking: Maintains state for all policies simultaneously
 * 4. Configurable adaptation interval: Controls how frequently to re-evaluate policy choice
 */
 ...
 * determining which materials to remove from the cache when it reaches capacity.
 * It includes several policy implementations optimized for different access patterns:
 * 
 ...
 * @details
 * 
 * The LRUEvictionPolicy class implements the classic Least Recently Used (LRU) cache
 * eviction strategy, which removes the least recently accessed items when the cache
 * reaches capacity. This is one of the most widely used cache replacement algorithms
 * due to its good performance across a wide range of workloads.
 * 
 * Key characteristics:
 ...
 * 3. O(1) time complexity: Constant time for all operations with proper implementation
...
private:
    ...
    std::unordered_map<void*, size_t> m_lastAccessMap; // Item to last access time
