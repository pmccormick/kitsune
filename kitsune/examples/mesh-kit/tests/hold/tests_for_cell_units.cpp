...
public:
  ...
  bool runAllTests() {
    ...
    std::cout << "\nUnit Tests Results: " << testsPassed << " of " << testsTotal
...
private:
  ...
  bool testVertexVelocityUnitConversions() {
    ...
    auto [vu_mph, vv_mph] =
        cell.getVertexVelocityWithUnits(Cell::VertexPosition::NORTHWEST, "mph");
    pass &= approxEqual(vu_mph, 22.37, 0.01);
    pass &= approxEqual(vv_mph, 11.185, 0.01);

    // Verify m/s to knot for vertex
    auto [vu_knot, vv_knot] = cell.getVertexVelocityWithUnits(
        Cell::VertexPosition::NORTHWEST, "knot");
    pass &= approxEqual(vu_knot, 19.44, 0.01);
    pass &= approxEqual(vv_knot, 9.72, 0.01);

    // Test setting vertex velocity with units
    cell.setVertexVelocityWithUnits(Cell::VertexPosition::NORTHEAST, 50.0, 25.0,
                                    "mph");
    auto [vu, vv] = cell.getVertexVelocity(Cell::VertexPosition::NORTHEAST);
    pass &= approxEqual(vu, 22.352, 0.001);
    pass &= approxEqual(vv, 11.176, 0.001);
  ...
  bool testRoundTripConversions() {
    ...
    for (const auto &unit : {"m/s", "mph", "knot"}) {
      double initial_u = 10.0;
      double initial_v = 5.0;
      cell.setVelocityU(initial_u);
      cell.setVelocityV(initial_v);
      double converted_u = cell.getVelocityUWithUnits(unit);
      double converted_v = cell.getVelocityVWithUnits(unit);
      cell.setVelocityWithUnits(converted_u, converted_v, unit);
      pass &= approxEqual(cell.getVelocityU(), initial_u, 0.01);
      pass &= approxEqual(cell.getVelocityV(), initial_v, 0.01);
    }
  ...
  bool testUnitBoundaryValues() {
    ...
    pass &= cell.getPressure() >
            101325000.0; // Should be higher than 1000 atm in Pa
    assert(pass == true);
...
int main() {
  ...
  if (allPassed) {
    ...
    std::cout << "\nSome unit tests failed. See above for details."
              << std::endl;
    return 1;
  }
}
