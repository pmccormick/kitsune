  ...
  void SetUp() override {
    ...
    TEST_F(MaterialTest, CustomPropertyFunction) {
      ...
      auto testMaterial = std::make_shared<Material>(
          Material::MaterialType::FLUID, "CustomFunc");
    ...
    TEST_F(MaterialTest, MaterialMixing) {
      ...
      // Test linear mixing
      EXPECT_NEAR(mixtureDensity, (waterDensity + airDensity) / 2.0, 0.1)
          << "Density should use linear mixing by default";
      ...
      EXPECT_NEAR(mixture2Density, waterDensity * 0.75 + airDensity * 0.25, 0.1)
          << "Density mixing should be proportional to fractions";
    ...
    TEST_F(MaterialTest, MixingRules) {
      ...
      // Linear mixing
      auto linearMix =
          Material::createMixture({m_water, m_oil}, {0.5, 0.5}, "linear");
      ...
      auto logMix =
          Material::createMixture({m_water, m_oil}, {0.5, 0.5}, "logarithmic");
      double logViscosity =
          logMix->getProperty(Material::MaterialProperty::DYNAMIC_VISCOSITY);
      EXPECT_NEAR(logViscosity,
                  exp(0.5 * log(waterViscosity) + 0.5 * log(oilViscosity)),
                  1e-6)
          << "Logarithmic mixing should use exponential average";

      // Harmonic mixing (common for thermal conductivity of perpendicular
      // layers)
      auto harmonicMix =
          Material::createMixture({m_water, m_oil}, {0.5, 0.5}, "harmonic");
      double waterK = m_water->getProperty(
    ...
    TEST_F(MaterialTest, UnitConversions) {
      ...
      // Set density in g/cm³
      testMaterial->setPropertyWithUnits(Material::MaterialProperty::DENSITY,
                                         0.8, "g/cm³");
      EXPECT_NEAR(
          testMaterial->getProperty(Material::MaterialProperty::DENSITY), 800.0,
          1e-6)
      ...
      testMaterial->setPropertyModel(
          ...
          {0.001}); // 0.1% increase per degree K
      testMaterial->setProperty(
