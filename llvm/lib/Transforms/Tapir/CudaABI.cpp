...
Function *CudaLoop::resolveLibDeviceFunction(Function *Fn) {
  ...
  // complex; e.g., printf().
  if (FnName == "printf" || FnName == "fprintf") {
...
/*
  ...
  if (Fn->isIntrinsic()) {
    ...
    if (Fn->getName() == "llvm.sqrt") {
      // TODO: Not exactly a fast-path here (given we typically
      // get here via -ffast-math).
      Function *nvi;
      if (Fn->getReturnType()->isFloatTy())
        nvi = Intrinsic::getDeclaration(&KernelModule,
                                        Intrinsic::nvvm_sqrt_f);
      else // TODO: This assumes double... SAFE?  fp16 could be a problem.
        nvi = Intrinsic::getDeclaration(&KernelModule,
                                        Intrinsic::nvvm_sqrt_rz_d);
      return nvi;
    } else if (Fn->getName() == "llvm.cos") {
      Function *nvi = Intrinsic::getDeclaration(&KernelModule,
                                Intrinsic::nvvm_cos_approx_f);
      return nvi;
    } else if (Fn->getName() == "llvm.sin") {
      Function *nvi = Intrinsic::getDeclaration(&KernelModule,
                                Intrinsic::nvvm_sin_approx_f);
      return nvi;
